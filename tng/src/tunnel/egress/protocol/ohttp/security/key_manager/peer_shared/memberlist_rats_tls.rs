...
impl KeyStore {
    ...
    pub async fn merge_received_keys(&self, keys: Vec<KeyInfo>, peer: &SocketAddr) {
        let now = SystemTime::now();
        let own = self.own_keys.read().await;
        ...
        for key_info in keys {
            if now < key_info.expire_at {
                if let Ok(pkd) = key_info.key_config.public_key_data() {
                    if !own.contains_key(&pkd) && !peer_keys.contains_key(&pkd) {
                        let has_private_key = key_info.key_config.dangerous_sk().is_some();
                        tracing::info!(?pkd, has_private_key, "new peer key");
                        new_key_count += 1;
                    }
                    if !own.contains_key(&pkd) {
                        peer_keys.insert(pkd, key_info);
                    }
                }
            }
        }
        peer_keys.retain(|pkd, ki| now < ki.expire_at && !own.contains_key(pkd));
        if new_key_count > 0 {
...
impl<R: Runtime> StreamLayer for RatsTls<R> {
    ...
    type Options = (Arc<RaContext>, TokioRuntime, Arc<KeyStore>);

    #[inline]
    async fn new((ra_context, runtime, key_store): Self::Options) -> io::Result<Self> {
        Ok(Self {
            forwarder: Arc::new(
    ...
    async fn connect(&self, addr: SocketAddr) -> io::Result<Self::Stream> {
        ...
        let all_keys = self.key_store.get_all_keys_for_exchange().await;
        send_keyset(&mut stream, &all_keys).await?;
        let peer_keys = recv_keyset(&mut stream).await?;
        self.key_store.merge_received_keys(peer_keys, &addr).await;
                                    ...
                                    match result {
                                        Ok((mut stream, _att)) => {
                                            ...
                                            let exchange_result: io::Result<()> = async {
                                                let peer_keys = recv_keyset(&mut stream).await?;
                                                let all_keys = key_store.get_all_keys_for_exchange().await;
                                                send_keyset(&mut stream, &all_keys).await?;
                                                key_store.merge_received_keys(peer_keys, &peer_addr).await;
                                                Ok(())
                                            }.await;
