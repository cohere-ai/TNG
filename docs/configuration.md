...
- **`rats_tls_pool_ttl`** (int, optional, default value is 600): How long a pooled `rats_tls` ingress session may be reused, in seconds. Independent of `refresh_interval`. A verify-only ingress (no `attest` block) uses this default. `0` disables reuse. Expired pool entries are evicted without force-closing in-flight requests.
...
- **`refresh_interval`** (int, optional, default value is 600): Specifies how often TNG rotates its own TLS key-carrier certificate on the `rats_tls` path (in seconds). If set to 0, a fresh certificate is generated for each new TLS config. This is key hygiene, not evidence freshness: evidence is produced during the post-handshake exchange. In Background Check mode this option has no effect when communicating using the OHTTP protocol. It does not control ingress session pooling; use `rats_tls_pool_ttl` for that.
...
- **`aa_type`** (string, optional, defaults to "uds"): Attestation Agent type. Possible values: "uds"
- **`aa_addr`** (string, required for "uds"): Attestation Agent unix socket address (e.g., "unix:///run/confidential-containers/attestation-agent/attestation-agent.sock")
- **`refresh_interval`** (int, optional, default value is 600): Specifies the frequency of obtaining attestation credentials (Attestation Token) from the Attestation Agent and Attestation Service (in seconds). If set to 0, it requests the latest Attestation Token each time a secure session is established. In Passport mode, on the `rats_tls` path this option now only affects TLS key-carrier certificate rotation, not evidence freshness. If communicating using the OHTTP protocol, this option affects the internal Attestation Token cache update frequency, but does not affect the OHTTP key rotation frequency. It does not control ingress session pooling; use `rats_tls_pool_ttl` for that.
...
- **`key`** (KeyConfig): OHTTP key management configuration.
    ...
    - **`join_max_attempts`** (`integer`, optional, default `1`): Maximum number of join attempts including the initial attempt. `1` means try once with no retries (default). `0` means unlimited retries (similar to serf-cli). Any value greater than 1 means try up to that many times total (initial + retries). When set to `1`, the `join_retry_interval` field is ignored.
